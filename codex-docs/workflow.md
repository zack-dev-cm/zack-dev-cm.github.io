...
`scripts/update-gonzo-paper-reviews.mjs` treats `https://t.me/gonzo_ML` as a
public discovery signal, then uses primary paper metadata for original English
notes. Do not translate or republish the channel review text.
...
Keep each review source-ledgered with the Gonzo ML post, primary paper, and PDF.
Avoid unsupported benchmark claims, copied abstracts, private feeds, API keys,
